...
module Rsa = struct
  ...
  let test_val ~decode nth (expected : Packet.Body.t) file ctxt =
    ...
    let (public_packet : Packet.t) = List.nth res nth in
    assert_equal ~ctxt public_packet.packet expected

  let test_rsa_pub1 =
    test_val
      ~decode:(fun x -> decode x [])
      0 public_packet (fixture "rsa_public.pgp")
  ...
  let test_val ~decode (expected : Packet.Body.t) file ctxt =
    let res = decode file in
    let (public_packet : Packet.t) = List.hd res in
    assert_equal ~ctxt public_packet.packet expected

  let test_rsa_public1 =
    test_val
      ~decode:(fun x -> decode x [])
      public_packet (fixture "rsa_public.pgp")

  let test_rsa_public2 ctxt =
    let res = decode (fixture "rsa_private.pgp") [] in
    let res_public = treat_secret1 res in
    assert_equal ~ctxt res_public public_key2

  let test_rsa_private ctxt =
    let res = decode (fixture "rsa_private.pgp") [] in
    let secret_value = List.hd res in
    assert_equal ~ctxt secret_packet secret_value.packet
...
module Dsa = struct
  ...
  let test_dsa_public1 =
    test_val
      ~decode:(fun x -> decode x [])
      public_packet (fixture "dsa_public.pgp")

  let test_dsa_secret =
    test_val
      ~decode:(fun x -> decode x [])
      secret_packet
      (fixture "dsa_private.pgp")
